...
df["current_date_time"] = df["current_date_time"].replace("2000-00-00 00:00:00", np.nan)
...
if len(nan_indices) > 0:
    ...
    nan_ranges = np.split(nan_indices, np.where(np.diff(nan_indices) != 1)[0] + 1)
...
for x in range(len(new_nan_ranges)):
    ...
    # Normal condition for wrong block data:
        ...
        for i in range(end_index, -1, -1):
            df.loc[(i), "current_date_time"] = df["current_date_time"][i + 1] - time_add
